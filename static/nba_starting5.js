...
async function searchPlayers(term) {
    ...
    const res = await fetch(`/api/nba_starting5/search?q=${encodeURIComponent(term)}${suffix}`);
    const data = await res.json();
...
async function selectName(name) {
    ...
    const res = await fetch(
        isOnlineMode()
            ? `/api/nba_starting5/years?player=${encodeURIComponent(name)}&game_id=${encodeURIComponent(state.game_id)}`
...
async function confirmYear() {
    ...
    const res = await fetch(
        isOnlineMode()
            ? `/api/nba_starting5/validate?player=${encodeURIComponent(state.pendingName)}&season=${season}&game_id=${encodeURIComponent(state.game_id)}&token=${encodeURIComponent(playerToken())}`
